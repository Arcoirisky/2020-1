...
int cr_read(crFILE* archivo, void* buffer, int nbytes){
    if(archivo == NULL){
    ...
    int total = nbytes;
    int punt = 0;
    int posicion = archivo->lectBuff->pByte;
    while(punt < archivo ->lectBuff->len_punteros){
        ...
        int pos_actual = leidos;//archivo->lectBuff->bytesLeidos;
        //printf("%i %i %i\n", pos_actual, nbytes, archivo->lectBuff->pByte);
        for(int i = 0; i < nbytes; i++){
            ((char*)buffer)[pos_actual + i] = archivo ->lectBuff->buffBloque[i + posicion];
            //printf("%c",((char*)buffer)[pos_actual + i]);
            leidos++;
            archivo->lectBuff->pByte++;
            archivo->lectBuff->bytesPorLeer--;
            archivo->lectBuff->bytesLeidos++; 
	        if(leidos >= total){
                //printf("%i %i %i %i\n",leidos, archivo->lectBuff->pByte,archivo->lectBuff->bytesPorLeer,archivo->lectBuff->bytesLeidos);
                return leidos;
            }
        }
...
int cr_read_fail(crFILE* file_desc, void* buffer, int nbytes){
    if(file_desc == NULL){
       printf("archivo nulo, no se puede leer nada...\n");
       return -1;
    }
