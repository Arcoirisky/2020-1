...
int main(int argc, char **argv)
{
  ...
  //printf("CREAMOS EL ARBOL CON LA IMAGEN...\n");
  Arbol* arbol;
  //Arbol* temporal;
  int nivel = 1;
  int x0 = 0;
  ...
  int z1 = img ->height;
  arbol = NULL;
  Insertar(&arbol,x0,z0,x1,z1,nivel, img);
  //Alturas(arbol); //<-- da muchos prints xD
  //int n = altura(arbol,nivel);
  //printf("N: %i\n", n);
  //printf("color L: %f\n",img ->pixels[x0][z0].L);

  //printf("TERMINAMOS DE CREAR EL ARBOL...\n");
  // ...
  ...
  if (string_equals(MODE, "filter"))
  {
    ...
    alpha = atof(PARAM);
    //printf("La desviación estándar máxima que puede tener un cuadrante es de %f \n", alpha);
  ...
  else if (string_equals(MODE, "compress"))
  {
    ...
    int h = atoi(PARAM);
    //printf("Mi árbol debe tener como máximo %d hojas\n", h);
    alpha = 0;
    int aux = 0;
    contar_hojas(arbol,alpha,&aux);
  ...
  //printf("PINTANDO~...\n");
  pintar_arbol(arbol, alpha, img);
  //printf("TERMINANDO DE PINTAR...\n");
  // ...
  ...
  img_square_paint(
      ...
      (img->height/2)/2,                             // Fila donde comienza el cuadrado
      (img->width/2)/2,                              // Columna donde comienza el cuadrado
      img->height/(2*2),                             // Largo del lado del cuadrado
